...
public class MergeSort {
	public class Comparer implements Comparator<Integer> {
		private String str;
	...
	private static final int sortTreshold = 48;
	...
	private static void parallelMerge(Integer[] arr, int start1, int end1, int start2, int end2, Integer[] dst, int pos,
		...
		if ((length1 + length2) < mergeTreshold) {
			...
			int xpos2 = Arrays.binarySearch(arr, start2, end2 - start2, arr[xpos1], comparer);
			...
			List<Callable<Object>> tasks = Arrays.asList(Executors.callable(() -> {
				parallelMerge(arr, start1, xpos1, start2, xpos2_f, dst, pos, comparer, executor);
			}), Executors.callable(() -> {
				parallelMerge(arr, xpos1 + 1, end1, xpos2_f, end2, dst, xpos_dst + 1, comparer, executor);
			}));
	...
	static void parallelSort(Integer[] src, int start, int end, Integer[] dest, Comparator<Integer> comparer,
		...
		if ((end - start) < sortTreshold) {
			Arrays.sort(src, start, end - start, comparer);
